...
def calculate_detection_statistics(datasets, measurement_model, track_gate):
    ais_statistics = dict()
...
def is_detected(est_list, measurements_all, measurement_timestamps, ownship_pose, measurement_model, gate):
    ...
    for k_est, k in enumerate(est_alive_indexes):#, measurements in enumerate(measurements_all):
        ...
        if np.linalg.norm(current_ais_velocity) < 1 or current_target_range > 1800:
            continue
...
def cluster_detections_angle(detections, angles, N_bins, symmetrize=False):
...
def cluster_detections_range(detections, target_range, N_bins):
...
def plot_angular_detections(ax, angles, P_D):
...
def plot_aspect_angle_vs_range(ax, angles, target_range):
    pass

def plot_num_scans(ax, angles, num_scans):
    pass
...
if __name__ == '__main__':
    ...
    radar_detection_data, ais_detection_data = calculate_detection_statistics(datasets, measurement_model, track_gate)
