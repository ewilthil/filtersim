...
clutter_model = autocommon.ConstantClutterModel(gen_scen.clutter_density)
...
def setup_ipda_manager(measurement_type=REAL):
    if measurement_type == REAL:
        ...
        measurement_model = autocommon.CartesianMeasurementModel(measurement_mapping, simulated_measurement_covariance, single_PD, clutter_model=clutter_model)
...
def setup_hmm_ipda_manager(measurement_type=REAL):
    ...
    if measurement_type == REAL:
        ...
        measurement_model = autocommon.CartesianMeasurementModelMarkovDetectionProbability(measurement_mapping, simulated_measurement_covariance, detection_model, clutter_model=clutter_model)
    tracker = autotrack.IPDAFTracker(target_model, measurement_model, track_gate, survival_probability)
    init = autoinit.IPDAInitiator(tracker, init_prob, conf_threshold, term_threshold)
    term = autotrack.IPDATerminator(term_threshold)
...
def setup_mc2_ipda_manager(measurement_type=REAL):
    if measurement_type == REAL:
        ...
        van_mc2_measurement_model = autocommon.CartesianMeasurementModel(measurement_mapping, simulated_measurement_covariance, [0, single_PD], clutter_model=clutter_model)
...
def setup_mcn_ipda_manager(measurement_type=REAL):
    if measurement_type == REAL:
        ...
        mc2_measurement_model = autocommon.CartesianMeasurementModel(measurement_mapping, simulated_measurement_covariance, [low_PD, high_PD], clutter_model=clutter_model)
