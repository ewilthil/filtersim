...
fname = 'low_nav_noiseN100'
errs = load_pkl(fname+'.pkl')
...
with plt.style.context(('filter')):
    ...
    con_ax.legend()
    plt.savefig('figs/NEES'+fname+'.pdf')


    pos_fig, pos_ax = plt.subplots(1,1)
    for j, err in enumerate(errs):
        pos_ax.plot(err.time, np.sqrt(np.mean(err.RMSE_pos, axis=0)), **markers[j])
    pos_ax.set_title('Position RMSE')
    pos_ax.set_xlabel('Time [s]')
    pos_ax.set_ylabel('RMSE [m]')
    pos_ax.legend()
    plt.savefig('figs/RMSE-pos-'+fname+'.pdf')
    ...
    for j, err in enumerate(errs):
        vel_ax.plot(err.time, np.sqrt(np.mean(err.RMSE_vel, axis=0)), **markers[j])
    vel_ax.set_title('Velocity RMSE')
    vel_ax.set_xlabel('Time [s]')
    vel_ax.set_ylabel('RMSE [m/s]')
    vel_ax.legend()
    plt.savefig('figs/RMSE-vel-'+fname+'.pdf')
